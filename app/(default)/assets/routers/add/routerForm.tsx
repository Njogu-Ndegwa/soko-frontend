...
// import { useCustomerForm } from './useRouterForm';
...
'use client'
// components/FormRouter.tsx
import { useEffect } from 'react';
import { ChevronLeft } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { useState } from 'react';
import { useRouterForm } from './useRouterForm';
import { useAlert } from '@/app/contexts/alertContext';
...
export default function FormRouter({ editData }: FormRouterProps) {
  const router = useRouter();
  const [isEditing] = useState(!!editData); // Determine if in edit mode
  const { alert } = useAlert();
  ...
  const [formData, setFormData] = useState({
    ...
    port: editData?.port?.toString() || '',
    username: editData?.username || '',
    password: editData?.password || '',
  });

  const { handleSubmit, isLoading, error } = useRouterForm({
    isEdit: isEditing,
    routerId: editData?.id,
    onSuccess: () => {
      router.back();
      alert({ text: `Router ${isEditing ? 'Updated' : 'Created'} Successfully`, type: "success" });
    },
    onError: () => {
      alert({ text: `There was a problem ${isEditing ? 'Updating' : 'Creating'} the Router`, type: "error" });
    }
  });
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-[96rem] mx-auto">
      <div className="mb-8">
        ...
        <h1 className="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">
          {isEditing ? 'Edit Router' : 'Create Router'}
        </h1>
      </div>
              ...
              <div className="mt-[25px]">
                ...
                >
                  {isLoading
                    ...
                    : `${isEditing ? 'Update' : 'Create'} Router`}
                </button>
              </div>
            </div>
          </div>
