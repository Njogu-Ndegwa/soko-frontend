...
export default function PaymentPlansTableWrapper() {
  return (
    <SelectedItemsProvider>
      <PaymentPlansTable />
    </SelectedItemsProvider>
...
function PaymentPlansTable() {
  const { data, loading: queryLoading, error: queryError, refetch } = useQuery(GET_MY_PLANS);
  ...
  const filteredPlans = plans.filter(plan =>
    ...
    plan.connectionType.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
    plan.speed.toLowerCase().includes(debouncedSearchTerm.toLowerCase())
  );
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-[96rem] mx-auto">
      <FeedbackModal
        ...
        setIsOpen={setDangerModalOpen}
        variant="danger"
        title={`Delete ${selectedItems.length} payment plan${selectedItems.length !== 1 ? 's' : ''}?`}
        content="Are you sure you want to delete the selected payment plan(s)? This action cannot be undone."
      ...
      <div className="sm:flex sm:justify-between sm:items-center mb-5">
        ...
        <div className="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
          ...
          />
          <Link
            href="/payments/payment-plans/add"
            className="btn bg-gray-900 text-gray-100 hover:bg-gray-800 dark:bg-gray-100 dark:text-gray-800 dark:hover:bg-white flex items-center justify-center"
          >
            <svg
              className="fill-current shrink-0 xs:hidden"
            ...
            <span className="max-xs:sr-only">Add Plan</span>
          </Link>
        </div>
      </div>
          ...
          <ul className="flex flex-wrap -m-1">
            <li className="m-1">
              <button className="inline-flex items-center justify-center text-sm font-medium leading-5 rounded-full px-3 py-1 border border-transparent shadow-sm bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-800 transition">
                All <span className="ml-1 text-gray-400 dark:text-gray-500">{filteredPlans.length}</span>
              </button>
            </li>
          </ul>
        </div>
